...
public abstract class Monster : CustomCollider
{
    ...
    bool fallDamageFlag;   //낙하데미지용 플래그
    ...
    public virtual void GroundedCheck()
    {
        ...
        else
        {
            ...
            if (grounded)
            {
                if (!fallDamageFlag && rigidbody2D.velocity.y < 0)
                {
                    if (rigidbody2D.velocity.y < -80)
                        Damage(maxHp);
                    else if (rigidbody2D.velocity.y < -60)
                        Damage((int)(maxHp * 0.5f));
                    else if (rigidbody2D.velocity.y < -40)
                        Damage(1);
                    fallDamageFlag = grounded;
                }
    ...
    void TryRoute()
    {
        if (monsterType == MonsterType.Dig || (Exception.IndexOutRange(nowPos, GroundManager.instance.linkArea) && GroundManager.instance.FindParent(GroundManager.instance.linkArea[nowPos.x, nowPos.y]) == 1))
        {
            //탐색경로도 갱신
