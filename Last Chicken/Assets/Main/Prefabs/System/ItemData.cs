...
public class ItemData : MonoBehaviour
{
    ...
    public void PointEnter()
    {
        ...
        if (GameManager.instance.playData.itemRecords[itemNum])
        {
            ...
            itemNameText.text += GameManager.instance.playData.language == Language.한국어 ? ItemManager.instance.itemData[itemNum].itemName : ItemManager.instance.itemData[itemNum].itemName_Eng;
            string temp = string.Empty;
            if (ItemManager.instance.itemData[itemNum].spawnShop)
            {
                for (int i = 0; i < 3; i++)
                {
                    string addString = GameManager.instance.playData.language == Language.한국어 ? ItemManager.instance.itemData[itemNum].shopItemExplain[i] : ItemManager.instance.itemData[itemNum].shopItemExplain_Eng[i];
            ...
            {
                string addString = GameManager.instance.playData.language == Language.한국어 ? ItemManager.instance.itemData[itemNum].itemExplain : ItemManager.instance.itemData[itemNum].itemExplain_Eng;
                temp += addString;
            }
