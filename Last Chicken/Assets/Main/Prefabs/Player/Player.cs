...
public class Player : CustomCollider
{
    ...
    void PlayerHang()
    {
        ...
        if (/*canHang && */fallTime > 0.2f)
        {
            ...
            bool hangCheck = IsAtTerrainbyHang(hangCollider, new Vector2(playerMoveDirection * 0.3f, 0));
        ...
        if (hang)
        {
            bool hangCheck = IsAtTerrainbyHang(hangCollider, new Vector2(hangDic * 0.3f, 0));
            if (!hangCheck)
                InitHang(hangDic);
            groundFallTime = 0;
        }
