...
public class ItemManager : ObjectPool
{
    ...
    private static Dictionary<KeyValuePair<int, Language>, string[]> itemExplain = new Dictionary<KeyValuePair<int, Language>, string[]>();
    ...
    #region[아이템 도감 데이터 읽기]
    private void ReadRecordData(Language language)
    {
        TextAsset textAsset = Resources.Load("Record/아이템도감_" + language.ToString()) as TextAsset;
        ...
        for (int r = 1; r < rowList.Count; r++)
        {
            ...
            if (!itemExplain.ContainsKey(new KeyValuePair<int, Language>(FindData(values[0]), language)))
            {
                string[] Data = { values[1], values[2] };
                itemExplain.Add(new KeyValuePair<int, Language>(FindData(values[0]), language), Data);
            }
        }
    }

    /// <summary> 아이템 도감 데이터 읽기 </summary>
    public string GetRecordData(string itemName, Language language, ItemSetting itemSetting)
    {
        return itemExplain[new KeyValuePair<int, Language>(FindData(itemName), language)][(int)itemSetting];
    }
    public string GetRecordData(int itemNum, Language language, ItemSetting itemSetting)
    {
        return itemExplain[new KeyValuePair<int, Language>(itemNum, language)][(int)itemSetting];
    }
    #endregion
