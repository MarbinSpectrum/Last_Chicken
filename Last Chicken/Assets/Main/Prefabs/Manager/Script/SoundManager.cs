...
public class SoundManager : MonoBehaviour
{
    ...
    void Awake()
    {
        if (instance == null)
        {
            ...
            smithy = Resources.Load("Sounds/SE/Smithy/대장간망치소리") as AudioClip;
    ...
    public void Update()
    {
        if (!AudioListener.pause && SE.volume == 0 && BGM.volume == 0)
    ...
    #region[배경음 정지재생]
    public void PlayBGM_Sound(bool b,bool sub = false)
    {
        if (!sub)
        {
            if (BGM)
    ...
    #region[플레이어 착지]
    public void PlayerLand(bool canStop = false)
    {
        if (!canStop)
            SE.PlayOneShot(playerLand);
        else
        {
            StopSE.volume = SE.volume;
            StopSE.PlayOneShot(playerLand);
        }
    }
    #endregion
    ...
    #region[대장간망치소리]
    public void Smithy(bool canStop = false)
    {
        if (!canStop)
            SE.PlayOneShot(smithy);
        else
        {
            StopSE.volume = SE.volume;
            StopSE.PlayOneShot(smithy);
        }
    }
    #endregion
