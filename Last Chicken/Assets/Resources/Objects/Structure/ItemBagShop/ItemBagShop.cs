...
public class ItemBagShop : AreaScript
{
    public static ItemBagShop instance;
    ...
    IEnumerator ReinforceAct()
    {
        ...
        for (int i = 0; i < GameManager.instance.playData.slotAct.Length; i++)
            if (!GameManager.instance.playData.slotAct[i])
            {
                GameManager.instance.playData.slotAct[i] = true;
                GameManager.instance.slotAct[i] = true;
                break;
            }
        ObjectInit();
