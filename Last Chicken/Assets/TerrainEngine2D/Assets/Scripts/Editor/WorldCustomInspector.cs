	...
	public class WorldCustomInspector : UnityEditor.Editor
	{
        ...
        void InitializeLists()
        {
            ...
            fluidTypesList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) => {
                ...
                EditorGUI.PropertyField(new Rect(rect.x, rect.y, rect.width, EditorGUIUtility.singleLineHeight), element, new GUIContent("Density: " + (index + 1)));
            };
		...
		public override void OnInspectorGUI ()
		{
            ...
            {
                serializedObject.FindProperty("worldData").objectReferenceValue = CreateInstance<WorldData>();
            ...
            switch (selectedTab)
            {
                case 0:
                    ...
                    objectFoldout = EditorGUILayout.Foldout(objectFoldout, "Objects", true, foldoutStyle);
                    if (objectFoldout)
                    {
                    ...
                    if (terrainFoldout)
                    {
                        ...
                        if (worldData.LoadWorld)
                        {
                            ...
                            if (!string.IsNullOrEmpty(worldData.WorldDirectory))
                            {
                                ...
                                if (!loaded) 
                                    EditorGUILayout.LabelField("Error loading file", EditorStyles.centeredGreyMiniLabel);
                                else
                                {
